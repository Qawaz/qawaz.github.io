...
export default function Page() {
    ...
    useLayoutEffect(()=>{
            ...
            trigger = ScrollTrigger.create({
                ...
                onUpdate(self) {
                    if (self.progress === 1 && self.direction > 0) {
                        ...
                        wrapBackward(self, seamlessLoop, scrub);
                    } else {
        ...
        document.getElementsByClassName("prev").item(0)!.addEventListener("click",()=>{
            scrubTo(scrub.vars.totalTime - spacing,trigger,seamlessLoop,scrub)
        })
        document.getElementsByClassName("next").item(0)!.addEventListener("click",()=>{
            scrubTo(scrub.vars.totalTime + spacing,trigger,seamlessLoop,scrub)
        })
    })
        ...
        <React.Fragment>
            <div className="gallery">
                ...
                <div className="actions">
                    <CustomButton className="prev">Prev</CustomButton>
...
function wrapBackward(trigger: globalThis.ScrollTrigger, seamlessLoop: gsap.core.Timeline, scrub: gsap.core.Tween) { // when the ScrollTrigger reaches the start again (in reverse), loop back to the end seamlessly
    iteration--;
...
function scrubTo(totalTime: number, trigger: globalThis.ScrollTrigger, seamlessLoop: gsap.core.Timeline, scrub: gsap.core.Tween) { // moves the scroll position to the place that corresponds to the totalTime value of the seamlessLoop, and wraps if necessary.
    let progress = (totalTime - seamlessLoop.duration() * iteration) / seamlessLoop.duration();
    if (progress > 1) {
        wrapForward(trigger);
    } else if (progress < 0) {
        wrapBackward(trigger, seamlessLoop, scrub);
    } else {
        trigger.scroll(trigger.start + progress * (trigger.end - trigger.start));
    }
}

function buildSeamlessLoop(items: any[], spacing: number) {
...
const CustomButton = styled.button`
  ...
  -webkit-box-shadow: 0 1px 0 #414141;
  -moz-box-shadow: 0 1px 0 #414141;
